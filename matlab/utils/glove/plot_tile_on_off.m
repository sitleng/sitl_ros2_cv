function ax = plot_tile_on_off(x, y, color, linewidth, box_start, box_end, wo_clutch, on_off_start, on_off_end)
    ax = nexttile;
    ...
    if ~isempty(on_off_start)
        xr = xregion(x(on_off_start), x(on_off_end), 'FaceColor', '#77AC30');
        for i = 1:numel(xr)
            xr(i).FaceAlpha = 0.1;
        end
    end
