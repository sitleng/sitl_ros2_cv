...
class PUB_KPT_CP(Node):
    ...
    def pub_psmjaw(self, g_psmjaw_cp):
    ...
    def callback(self, psm_msg, kpt_msg):
        g_psmtip_temp = tf_utils.posestamped2g(psm_msg)
        g_psmtip = self.g_ecmopencv_ecmdvrk.dot(g_psmtip_temp)
        ...
        if self.inst_name == "PCH":
            ...
            # )
            g_psmjaw = g_psmtip.dot(tf_utils.g_psm1tip_psm1jaw)
        elif self.inst_name == "FBF":
            g_psmjaw = g_psmtip.dot(tf_utils.g_psm2tip_psm2jaw)
        self.pub_psmjaw(g_psmjaw)
