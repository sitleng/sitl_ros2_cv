...
class PSM_HAND_CTRL():
    ...
    def set_duration(self, rot_distance):

            if rot_distance > 0.5:
                rot_distance = 0.5
            duration = (2e-3*rot_distance)+ 0.0015
        ...
        while not rospy.is_shutdown():
            if hand_app.flag_on_off == True:
                ...
                if (first) or (hand_app.bool_dist_real == True) :
                    ...
                    # distance =  abs(hand_app.angle-precendent_angle)
                    # hand_app.set_duration(distance)
                    hand_app.arm.run_jaw_servo_jp(
                        math.radians(hand_app.angle),
                        duration = 0.01
                    )
            ...
            precendent_angle =  hand_app.angle
