...
class PSM_HAND_CTRL():
    ...
    def __init__(self):
        ...
        self.arm_ik = ik_devel_utils.dvrk_custom_ik(
            ...
            self.params["TransWeight"],
            self.params["RotWeight"],
            self.params
        )
    ...
    def callbackTracker_frame(self, tracker_transform):
        ...
        self.x = tracker_transform.transform.translation.x     
        self.y = tracker_transform.transform.translation.y
        self.z = tracker_transform.transform.translation.z      
    ...
    def set_duration(self, rot_distance, distance):
            ...
            if rot_distance > 0.5:
                rot_distance = 0.5
            duration = (2e-1*rot_distance + 2*distance)/2 + 0.0015
...
if __name__=="__main__":
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if hand_app.flag_on_off == True:  
                ...
                s = time.time()
                goal_jp = hand_app.arm_ik.get_goal_jp(hand_app.arm.get_jp())
                # print("IK Time Elapsed: {}".format(time.time()-s))

                
                if (first) or (hand_app.bool_dist_real == True) :
                    hand_app.arm.run_full_servo_jp(  
                        goal_jp,
                        math.radians(hand_app.angle),
                        duration = 3
                    )
    ...
    # =================== HARD CODING ===================
# overwrite last three joints
# Extract Euler angles (in radians) using yaw_pitch_roll property
...
# print (yaw_degrees,roll_degrees)
...
# goal_jp[3] = math.radians(yaw_degrees)
# goal_jp[4] = math.radians(roll_degrees)
# goal_jp[5] = math.radians(-pitch_degrees)
